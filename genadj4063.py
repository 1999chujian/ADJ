...
timestamps = []
timestamps += utils.gen_timestamp(9, 30, interval=1, step=120 + 1)
timestamps += utils.gen_timestamp(13, 1, interval=1, step=117)
timestamps += utils.gen_timestamp(15, 0, interval=1, step=1)
...
def load_adj(args):
    time = args
    adj = pd.read_csv(f'/dfs/dataset/190-1705298898613/data/StockIndustry/{str(time)[:4]}/{str(time)[:8]}.csv',
                      index_col=0)
    # codes_list = df1[df1['datetimestamp'] == time]['code'].unique().tolist()
    codes_list = df_codes.tolist()
    # new_adj = adj.reindex(index=codes_list, columns=codes_list, fill_value=0)
    # print(time)
    # new_adj = new_adj.values
    new_adj = adj.reindex(index=codes_list, columns=codes_list, fill_value=0)
    new_adj = new_adj.values
    f = open(f'/dfs/dataset/1704939107951/data/adj_4063/Industry_{time}_1min4063.dat', 'wb')
...
elations = load_all_adjs(datetimestamps)
print('结束取图')
