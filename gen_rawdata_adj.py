...
def generator(datetimestamps):
    with Pool(processes=8) as pool:  # 选择一个适当的进程数量
...
RAW_DATA.to_parquet('/dfs/dataset/1704939107951/data/rawdata/RAW_DATA.parquet')
...
def load_adj(args):
    ...
    f= open(f'/dfs/dataset/1704939107951/data/adj/Industry_{time}.dat', 'wb')
    f.write(new_adj.astype(np.float32).tobytes())
    f.flush()
    del adj, new_adj

def load_all_adjs(unique_times, codes_list):
    with Pool(processes=152) as pool:
        time_codes_pairs = [(time, codes_list) for time in unique_times]
...
train_x,train_y,train_t= train_data.drop(other_cols, axis=1), train_data['yhat_raw_ret_v2v_1d'],train_data['datetimestamp']  
test_x,test_y ,test_t= test_data.drop(other_cols, axis=1), test_data['yhat_raw_ret_v2v_1d'],test_data['datetimestamp']

tensor_train_x,tensor_train_y= torch.from_numpy(train_x.values).float(),torch.from_numpy(train_y.values).float()
tensor_test_x,tensor_test_y = torch.from_numpy(test_x.values).float(), torch.from_numpy(test_y.values).float()

unique_times = np.concatenate((train_t.unique(), test_t.unique()))

print('开始取图')
relations = load_all_adjs(unique_times, codes_list)
print('结束取图')
